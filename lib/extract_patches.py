...
def extract_random(full_imgs,full_masks, patch_h,patch_w, N_patches, inside=True):
    if (N_patches%full_imgs.shape[0] != 0):
        ...
        print( ("N_patches: please enter a multiple of" + str(full_imgs.shape[0])))
        exit()
...
#     patches_masks = np.empty((N_patches,full_masks.shape[1],patch_h,patch_w))
    ...
    for i in range(full_imgs.shape[0]):  #loop over the full images
        ...
        while k <patch_per_img:
            ...
            keepOrNot = random.randint(1,10)
            if (np.average(patch_mask) > 0.95) and (keepOrNot > 4):
                # do nothing
                print( "skip")
            else:
...
def extract_ordered_overlap(full_imgs, patch_h, patch_w,stride_h,stride_w):
    ...
    print( "Total patches extracted: " +str(N_patches_img) )
    patches = np.empty((N_patches_tot,full_imgs.shape[1],patch_h,patch_w))
    iter_tot = 0   #iter over the total number of patches (N_patches)
    for i in range(full_imgs.shape[0]):  #loop over the full images
